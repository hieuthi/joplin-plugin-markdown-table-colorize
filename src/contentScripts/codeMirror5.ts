...
function calcState(line: string) {
	let ret = {src: line, bMark:0, eMark: 0, tShift:0, sCount: 0 };
	let s = ret.src;
	let indent_found = false;
	for (
        let start = 0, pos = 0, indent = 0, offset = 0, len = s.length;
        pos < len;
        pos++
    ) {
...
function plugin(CodeMirror: any) {
    if (CodeMirror.cm6) return;

	CodeMirror.defineOption("mdTableColorize", false, async function(cm: any, val: boolean, old: boolean) {
	...
	function colorizeRow(cm: any, idx: number, nColumns: number, lineState = null, rclass = null) {
		...
		if (firstCh === 0x7C/* | */ ) {
			colStart = firstChPos;
			const pipeIdx  = lineState.src.substring(colStart+1).indexOf("|");
		...
		while (colEnd < lineState.eMark) {
			col++;
			colStart = colEnd + 1;
			const pipeIdx = lineState.src.substring(colStart).indexOf("|");
		...
		cm.operation(function() {
			...
			for (var idx = startIdx; idx <= endIdx; idx++) {
				...
				if (nColumns < 1) {
					...
					const firstCh = nextLine.src.charCodeAt(pos++);
					if (firstCh !== 0x7C/* | */ && firstCh !== 0x2D/* - */ && firstCh !== 0x3A/* : */) { continue; }
					if (pos >= nextLine.eMark) { continue; }

					const secondCh = nextLine.src.charCodeAt(pos++);
					...
					while (pos < nextLine.eMark) {
						let ch = nextLine.src.charCodeAt(pos);
